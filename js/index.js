...
if (bet == null) {
  $msg.html('VOER EEN GELDIG BEDRAG IN!')
} else {
var stawka = $('#stawka').val();
var saldo = $saldor.html();
if (+stawka < 100) {
  $msg.html('JE MOET MINIMUM €100 INZETTEN')
} else if (+stawka > max_bet) {
  $msg.html('JE MAG MAXIMUM €200000 EURO INZETTEN')
} else {
  if (+stawka > +saldo) {
    $msg.html('JE HEBT NIET GENOEG CONTANT GELD')
  } else {
  foo.play();
  var wlasciwesaldo = +saldo - +stawka
  $saldor.html(wlasciwesaldo);
  $saldoh.html('Saldo: '+wlasciwesaldo+'€');
  $.post('http://esx_roulette/lose', JSON.stringify({
    amount: +stawka,
  }));
 ...
 var  randomNumber = Math.floor(Math.random() * 36),
  ...
  setTimeout(function() {
    ...
    if (color == bet) {
      $thisResult = '<li style="color:#ffbf00;" class="previous-result color-'+ color +'"><span class="previous-number">'+ randomNumber +'</span><span class="previous-color">GEWONNEN</span></li>';
      $msg.html('<span style="color:#ffbf00;">JE HEBT GEWONNEN: '+wygrana+'$');
      ...
      $msg.html('JE BENT VERLOREN');
      $thisResult = '<li class="previous-result color-'+ color +'"><span class="previous-number">'+ randomNumber +'</span><span class="previous-color">VERLOREN</span></li>';
        losdio.play()
      console.log('Persoon verloor '+tolose)
    }
...
document.onkeyup = function(data){
	if (data.which == 27){
		...
		$.post('http://esx_roulette/NUIFocusOff', JSON.stringify({}));
	}
}
